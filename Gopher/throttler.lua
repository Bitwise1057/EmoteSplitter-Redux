   ...
   local hardware = Me.inside_hook or Me.triggered_from_keystroke
	...
	elseif msgtype == "CLUBEDIT" then
      ...
      if not hardware then
         ...
         -- SAY, YELL, and EMOTE are blocked universally without a hardware
         -- event as of 8.2.5 -- this applies inside instances too, so the
         -- earlier IsInInstance() carve-out is removed to fix the
         -- ADDON_ACTION_FORBIDDEN error thrown when split/queued emotes are
         -- dispatched via C_Timer callbacks in combat.
         if msg.type == "CHANNEL"
            or msg.type == "SAY"
            or msg.type == "YELL"
		...
		-- global, so it is clean and untainted. This call happens from a
		-- C_Timer callback (for queued chunks 2..N), which is outside the
		-- editbox hardware-event chain but is not subject to the lockdown
		-- restriction for non-hardware-restricted types.
      Me.addon_action_blocked = false
