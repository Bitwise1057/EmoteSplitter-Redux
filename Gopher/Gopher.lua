...
Me.event_hooks = {    -- functions.
   ...
   -- Fired when a chat messaging lockdown begins or ends (boss, M+, PvP).
   -- While locked down Gopher steps aside and Blizzard handles the send.
...
function Me.OnLogin()
   ...
   -- v12: Catch addon blocked event.
   Me.frame:RegisterEvent( "ADDON_ACTION_BLOCKED" )

   -- 12.0: Track encounter lockdown for user notification.
...
function Me.QueueChat( msg, type, arg3, target )
   ...
   -- When called from the editbox pre-send hook, the very first chunk needs
   -- to be delivered by setting the editbox text (so Blizzard's own untainted
   -- SendChatMessage call handles it). We capture it here and skip queueing.
   if Me.capturing_first_chunk and Me.editbox_first_chunk == nil then
      Me.editbox_first_chunk = msg
      -- Mark capture done so subsequent chunks queue normally.
...
function Me.OnChatSent( msg )
   ...
   -- 12.0 (Midnight): CHAT_MSG_SAY/EMOTE/YELL no longer echo back for the
   -- player's own messages in many contexts, so TryConfirm never fires and
   -- the queue times out. We start a short auto-confirm timer as a fallback.
   -- A real throttle error arriving via CHAT_MSG_SYSTEM will call ChatFailed
   -- which cancels this timer (via "push" mode) before it fires, so we don't
   -- falsely confirm a failed send.
   if channel == 1 then
...
-- been committed from our queue.
--
-- kind: Type of chat message the event handles. e.g. SAY, EMOTE, etc.
--
-- Note: In 12.0 the sender argument from CHAT_MSG_SAY etc. arrives as a
-- tainted string that cannot be compared against secure values. We therefore
-- confirm blindly on chat type match, which is safe because the auto-confirm
-- timer in OnChatSent already handles the case where no echo arrives at all.
...
-- Hooks
-------------------------------------------------------------------------------
-- 12.0 Rewrite: Gopher no longer replaces C_ChatInfo.SendChatMessage.
-- Replacing that global taints it permanently, causing ADDON_ACTION_FORBIDDEN
-- for all subsequent calls from within the tainted execution chain.
--
-- Instead, we hook ChatFrame.OnEditBoxPreSendText -- a Blizzard-provided
-- pre-send callback that fires before the editbox submits its text. Inside
-- that callback we are still within the secure hardware-event call chain, so
-- we can safely modify the editbox text to the first chunk and let Blizzard's
-- own untainted code call SendChatMessage for us. Remaining chunks go into
-- Gopher's existing queue, which calls C_ChatInfo.SendChatMessage directly
-- (not through a hook, so no taint is involved).
--
-- When a chat messaging lockdown is active (boss encounter, Mythic+, rated
-- PvP) we do nothing at all -- Blizzard's own path handles the send cleanly.
-- Messages over 255 chars will be server-truncated during lockdown, which is
-- the same behaviour as having no chat addon installed.
--
-- BNSendWhisper and C_Club.SendMessage are still replaced below because they
-- do not participate in the editbox send chain and are not subject to the
-- same taint restrictions.
Me.hooks = Me.hooks or {}

-------------------------------------------------------------------------------
-- Returns true when Gopher should step aside and let Blizzard handle the
-- send without any intervention.
function Me.IsLocked()
   -- InCombatLockdown covers the secure execution restriction.
   -- C_ChatInfo.InChatMessagingLockdown covers the 12.0 boss/M+/PvP lockdown.
...
-------------------------------------------------------------------------------
-- Pre-send editbox hook. Fires before the editbox calls SendText, while we
-- are still inside the secure OnEnterPressed hardware-event call chain.
-- This is the new interception point replacing the SendChatMessage hook.
...
if not Me.editbox_hooked then
   ...
   EventRegistry:RegisterCallback(
      ...
      function( _, editBox )
         -- If locked down or Gopher is suppressed, do nothing.
         if Me.IsLocked() then return end

         local msg = editBox:GetText()
         if not msg or msg == "" then return end

         -- Determine chat type the same way Chattery does.
         local chat_type = ChatFrameUtil.GetActiveChatType()
         chat_type = chat_type and chat_type:upper() or "SAY"

         -- Get chunk size for this type.
         local chunk_size = Me.chunk_size_overrides[chat_type]
                            or Me.default_chunk_sizes[chat_type]
                            or Me.chunk_size_overrides.OTHER
                            or Me.default_chunk_sizes.OTHER

         -- Only intercept if the message actually needs splitting or
         -- processing (newlines count too).
         local needs_split = msg:len() > chunk_size
                             or msg:find( "\n" ) or msg:find( "\\n" )

         -- Fire CHAT_NEW listeners so other addons (Tongues etc.) can
         -- transform the message before we decide how to split it.
         -- We need language and target from the editbox.
         local language = editBox.languageID
         local target   = editBox:GetTellTarget()
                          or editBox:GetChannelTarget()
         if target == 0 then target = nil end

         -- Run the message through Gopher's full AddChat pipeline, but
         -- suppress the first chunk -- we will deliver it by setting the
         -- editbox text, letting Blizzard send it cleanly. Remaining chunks
         -- go to Gopher's queue as normal.
         -- We set a flag so QueueChat knows chunk[1] is pre-delivered.
         ...
         if Me.editbox_first_chunk then
            -- Replace editbox text with first chunk only. Blizzard's own
            -- SendText will call SendChatMessage with this clean value.
            editBox:SetText( Me.editbox_first_chunk )
            Me.editbox_first_chunk = nil
         end
         -- If editbox_first_chunk was never set, Gopher found nothing to
         -- split (e.g. message was cancelled by a CHAT_NEW listener) and
         -- the editbox text is left unchanged.
      end
   )
end
