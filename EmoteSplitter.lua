...
--	end
	...
	-- We save each editbox's original limits on first focus and restore them
	-- when chat is locked down. This mirrors Chattery's approach: if Gopher
	-- cannot intercept the send (lockdown active), the editbox must enforce the
	-- server's 255-byte limit itself so oversized messages never reach SendText.
	Me.editbox_defaults = Me.editbox_defaults or {}

	EventRegistry:RegisterCallback(
		"ChatFrame.OnEditBoxFocusGained",
		function( _, editBox )
			-- Save original limits the first time we see this editbox.
			...
			if Gopher.Internal.IsLocked() then
				-- Chat is locked down -- restore defaults so the editbox
				-- enforces the 255-char limit and prevents the
				-- "Chat message limits exceeded" error.
				...
				-- Normal operation -- remove limits so Gopher can split.
...
-- Respects lockdown state the same way the focus handler does.
function Me.ChatEdit_OnShow( editbox )
	if Gopher.Internal.IsLocked() then
		-- Don't touch limits during lockdown.
		return
	end
	editbox:SetMaxLetters( 0 )
	editbox:SetMaxBytes( 0 )
...
-------------------------------------------------------------------------------
-- 12.0: Chat messaging lockdown has started (boss encounter, Mythic+, rated
-- PvP). Gopher steps aside entirely during lockdown -- Blizzard handles the
-- send without any intervention. Messages over 255 chars will be truncated
-- by the server, same as if the addon were disabled.
function Me.Gopher_ENCOUNTER_LOCKDOWN_START()
	print( "|cffff9900<EmoteSplitter>|r "
	    .. "Message splitting paused (encounter lockdown). "
	    .. "Chat works normally; 255 character limit applies." )
end

-------------------------------------------------------------------------------
-- Lockdown is over. Full splitting and queuing resumes.
